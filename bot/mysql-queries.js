...
async function guildExists(guildID) {
  ...
  const [guildResults] = await mysql.pool.query(
...
async function addGuild(guild) {
  ...
  const insertGuild =
    "INSERT INTO guilds (guildId, guildName, shortID) " + "VALUES (?, ?, ?)";
  try {
    await mysql.pool.query(insertGuild, [
      guild.guildID,
      guild.guildName,
      guild.shortID,
    ]);
...
async function addChannels(channels) {
  ...
  for (const ch of channels) {
    try {
      ...
      await mysql.pool.query(insertChannel, [
        ch.channelID,
        ch.channelName,
        ch.guildID,
      ]);
...
async function addMembers(members) {
  ...
  for (const mem of members) {
    try {
      await mysql.pool.query(insertMember, [
        mem.userID,
        mem.guildID,
        mem.userName,
        mem.userNick,
...
async function addMessages(messages) {
  ...
  for (const msg of messages) {
    try {
      ...
      await mysql.pool.query(insertMessage, [
        msg.messageID,
        msg.channelID,
        msg.guildID,
        msg.userID,
...
async function addAttachments(attachments) {
  ...
  for (const att of attachments) {
    try {
      ...
      await mysql.pool.query(insertAttachment, [
        att.attachmentID,
        att.messageID,
        att.attType,
        att.attName,
...
async function getShortID(guildId, web_domain, prefix) {
  ...
  try {
    ...
    const [results] = await mysql.pool.query(selectShortId, [guildId]);
    if (results.length == 0) {
...
async function getThenAddMember(member) {
  const getMember =
    "SELECT * FROM members WHERE members.userID = ? AND members.guildID = ?";
  try {
    // Check if member already in database
    const [results] = await mysql.pool.query(getMember, [
      member.userID,
      member.guildID,
    ]);
